...
def _meta_path(save_dir):
  """Get meta path."""
  meta_prefix = "meta.{}.sent".format(FLAGS.split)
...
def _create_data(input_paths, tokenizer):
  ...
  for input_path in input_paths:
    input_data = data_utils.read_sents(input_path, tokenizer)

    if input_data:
      total_line_cnt += len(input_data)
      all_data.extend(input_data)

  tf.logging.info("[Task %d] Total number line: %d", FLAGS.task, total_line_cnt)

  create_orderless_lm_tfrecords(
...
def create_orderless_lm_tfrecords(save_dir, data):
  """create orderless language modeling tfrecords from numpy array."""
  tf.logging.info("===== Create orderless LM tfrecords =====")
