...
try:
  from google3.experimental.users.zihangd.lm import tpu_optimizer
...
flags.DEFINE_string("optimizer", default="adam",
...
flags.DEFINE_float("finetune_lr_ratio", 1.0,
                   "The actual learning rate for finetuning-only components is "
                   "learning_rate x finetune_lr_ratio.")
...
flags.DEFINE_integer("batches_per_update", default=1,
                     help="Used to simulate large-batch training. Actual "
                     "batch size is `train_batch_size x batches_per_update`.")
...
def get_train_op(total_loss):
  ...
  if FLAGS.dynamic_clip:
    optimizer = dynamic_clip.DynamicClipOptimizer(optimizer)

  if FLAGS.batches_per_update > 1:
    optimizer = deferred_grad_optimizer.GradientAggregationOptimizer(
        optimizer, FLAGS.batches_per_update)

  if FLAGS.use_tpu:
    if FLAGS.per_core_clip:
      optimizer = tpu_optimizer.CrossShardOptimizer(optimizer)
    else:
      optimizer = tpu_optimizer.CrossShardOptimizer(optimizer, clip=FLAGS.clip)
  ...
  if FLAGS.batches_per_update == 1 and FLAGS.optimizer in ["adamw", "lamb"]:
